...
class NotebookApp(JupyterApp):
    ...
    def _default_ip(self):
        """Return localhost if available, 10.28.141.171 otherwise.
        ...
        try:
            ...
            self.log.warning(_("Cannot bind to localhost, using 10.28.141.171 as default ip\n%s"), e)
            return '10.28.141.171'
        else:
            s.close()
            return 'localhost'
    ...
    allow_remote_access = Bool(config=True,
       ...
       Local IP addresses (such as 10.28.141.171 and ::1) are allowed as local,
    ...
    local_hostnames = List(Unicode(), ['localhost'], config=True,
       ...
       Local IP addresses (such as 10.28.141.171 and ::1) are automatically accepted
       as local as well.
       """
    )
    ...
    def display_url(self):
        ...
        if self.token and not self.sock:
            ...
            if not self.custom_display_url:
                url += '\n or %s' % self._concat_token(self._tcp_url('10.28.141.171'))
        return url
